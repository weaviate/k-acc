            ...
            {products.map((product: Product, index: number) => (
                <div className="relative py-2 overflow-hidden" key={product.id}>
                    ...
                    <div className="flex flex-col justify-left items-left ml-5 mt-2">
                        <h1 className="text-sm font-bold font-outfit text-primary mb-1">{product.name}</h1>
                        <h1 className="text-xs font-outfit text-primary mb-2">${product.price}</h1>
                        <p className="text-xs text-primary line-clamp-2">{product.description}</p>
                    </div>
